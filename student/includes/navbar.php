 <nav class="main-header navbar navbar-expand navbar-green navbar-dark">
    ...
    <ul class="navbar-nav">
      ...
      <li class="nav-item d-none d-sm-inline-block">
        <a href="files.php" class="nav-link">Home</a>
      </li>
    </ul>
	<?php
	$sql = "SELECT * FROM upload_files u 
			...
			WHERE f.TO_USERID ='".$_SESSION['userid']."' AND DATE(f.SHARE_DATETIME)=CURDATE()";
      ...
      <li class="nav-item dropdown">
		...
		<?php
			 $sql = "SELECT * FROM upload_files u 
					INNER JOIN tbladmin a ON u.UPLOADER=a.id
					INNER JOIN tblfileshare f ON u.FILEID=f.FILE_SHAREID 
					WHERE f.TO_USERID ='".$_SESSION['userid']."' AND DATE(f.SHARE_DATETIME)=CURDATE() LIMIT 5";
			$query = $conn->query($sql);
